...
const char *ota_url = "https://github.com/lekstar/CYD-Klipper_rus/blob/master/OTA.json"; // Prod url
...
void do_update_callback(int offset, int totallength)
{
    ...
    switch(global_config.language){
        case 1:
            lv_label_set_text_fmt(update_label, "%d/%d байт", offset, totallength);
            break;
        default:
...
void ota_do_update(bool variant_automatic)
{
    ...
    switch(global_config.language){
        case 1:
            lv_label_set_text(label, "OTA обновление...");
            break;
        default:

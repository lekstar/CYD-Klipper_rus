...
static void btn_no_reuse_password(lv_event_t * e)
{
    switch(global_config.language){
        case 1:
            lv_create_keyboard_text_entry(keyboard_cb_enter_password, "Введите пароль WiFi", LV_KEYBOARD_MODE_TEXT_LOWER, CYD_SCREEN_WIDTH_PX * 0.75, 63, "", false);
            break;
        default:
...
void ask_reuse_password(const char * ssid){
    ...
    switch(global_config.language){
        case 1:
            lv_label_set_text_fmt(label, "Использовать сохраненный пароль?\n(Длина пароля: %d)", strlen(global_config.wifi_password));
            break;
        default:
...
static void wifi_btn_manual_ssid(lv_event_t * e){
    switch(global_config.language){
        case 1:
            lv_create_keyboard_text_entry(wifi_keyboard_cb_manual_ssid, "Введите вручную имя сети WiFi", LV_KEYBOARD_MODE_TEXT_LOWER, CYD_SCREEN_WIDTH_PX * 0.75, 31, "", false);
            break;
        default:
...
void wifi_init_inner(){
    ...
    switch(global_config.language){
        case 1:
            lv_label_set_text(label, "Поиск сетей WiFi...");
            break;
        default:
    ...
    switch(global_config.language){
        case 1:
            lv_label_set_text(label, "Выбрать сеть");
            break;
        default:

...
static void set_fan_speed_text(lv_event_t * e) {
    ...
    switch(global_config.language){
        case 1:
            sprintf(data, "Вент.: %.0f%%", printer.fan_speed * 100);
            break;
        default:
...
static void set_speed_mult_text(lv_event_t * e){
    ...
    switch(global_config.language){
        case 1:
            sprintf(data, "Скорость: %.0f%%", printer.speed_mult * 100);
            break;
        default:
...
static void set_extrude_mult_text(lv_event_t * e){
    ...
    switch(global_config.language){
        case 1:
            sprintf(data, "Поток: %.0f%%", printer.extrude_mult * 100);
            break;
        default:
...
static void label_filament_used_m(lv_event_t * e){
    ...
    switch(global_config.language){
        case 1:
            sprintf(filament_buff, "%.2f м", printer.filament_used_mm / 1000);
            break;
        default:
...
static void label_total_layers(lv_event_t * e){
    ...
    switch(global_config.language){
        case 1:
            sprintf(layers_buff, "%d из %d", printer.current_layer, printer.total_layers);
            break;
        default:
...
static void label_pressure_advance(lv_event_t * e){
    ...
    switch(global_config.language){
        case 1:
            sprintf(pressure_buff, "%.3f (%.2fc)", printer.pressure_advance, printer.smooth_time);
            break;
        default:
...
static void label_feedrate(lv_event_t * e){
    ...
    switch(global_config.language){
        case 1:
            sprintf(feedrate_buff, "%d мм/с", printer.feedrate_mm_per_s);
            break;
        default:
...
void stats_panel_init(lv_obj_t* panel) {
    ...
    switch(global_config.language){
        case 1:
            create_stat_text_block(left_panel, "Координаты:", label_pos);

            if (printer.state != PRINTER_STATE_IDLE){
                create_stat_text_block(left_panel, "Потрачено филамента:", label_filament_used_m);
                create_stat_text_block(left_panel, "Слой:", label_total_layers);
            }

            create_stat_text_block(left_panel, "PA/(Smooth Time):", label_pressure_advance);
            create_stat_text_block(left_panel, "Подача:", label_feedrate);

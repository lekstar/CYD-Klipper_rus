        ...
        case 1:
            switch (get_current_printer_config()->show_stats_on_progress_panel)
            {
                case SHOW_STATS_ON_PROGRESS_PANEL_LAYER:
                    sprintf(buff, "Слой %d из %d", printer.current_layer, printer.total_layers);
                    break;
                case SHOW_STATS_ON_PROGRESS_PANEL_PARTIAL:
                    sprintf(buff, "Координаты: X%.2f Y%.2f\nПодача: %d мм/с\nПотрачено филамента: %.2f м\nСлой %d из %d", 
                ...
                case SHOW_STATS_ON_PROGRESS_PANEL_ALL:
                    sprintf(buff, "PA (Smooth time): %.3f (%.2fc)\nКоординаты: X%.2f Y%.2f Z%.2f\nПодача: %d мм/с\nПотрачено филамента: %.2f м\nВент.: %.0f%%\tСкорость: %.0f%%\nПоток: %.0f%%\tСлой %d из %d", 
...
void progress_panel_init(lv_obj_t* panel){
    ...
    if (global_config.show_estop){
        ...
        switch(global_config.language){
            case 1:
                lv_label_set_text(label, LV_SYMBOL_POWER " АВАРИЙНАЯ КНОПКА");
                break;
            default:

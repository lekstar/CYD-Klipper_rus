...
static void update_printer_data_hotend_temp(lv_event_t * e){
    ...
    switch(global_config.language){
        case 1:
            sprintf(hotend_buff, "Хотенд: %.0f C (Цель: %.0f C)", printer.extruder_temp, printer.extruder_target_temp);
            break;
        default:
...
static void update_printer_data_bed_temp(lv_event_t * e){
    ...
    switch(global_config.language){
        case 1:
            sprintf(bed_buff, "Стол: %.0f C (Цель: %.0f C)", printer.bed_temp, printer.bed_target_temp);
            break;
        default:
...
static void show_keyboard_with_hotend(lv_event_t * e){
    ...
    switch(global_config.language){
        case 1:
            lv_create_keyboard_text_entry(keyboard_callback, "Задать темп.хотенда");
            break;
        default:
...
static void show_keyboard_with_bed(lv_event_t * e){
    ...
    switch(global_config.language){
        case 1:
            lv_create_keyboard_text_entry(keyboard_callback, "Задать темп. стола");
            break;
        default:
...
static void set_temp_via_preset(lv_event_t * e){
    ...
    if (temp_edit_mode) {
        ...
        switch(global_config.language){
            case 1:
                lv_create_keyboard_text_entry(keyboard_callback, "Устан.пресет темп.");
                break;
            default:
...
void temp_panel_init(lv_obj_t * panel){
    ...
    switch(global_config.language){
        case 1:
            lv_label_set_text(label, LV_SYMBOL_DOWN " Экструд.");
            break;
        default:
    ...
    switch(global_config.language){
        case 1:
            lv_label_set_text(label, "Выкл.нагрев");
            break;
        default:
    ...
    switch(global_config.language){
        case 1:
            lv_label_set_text(label, "Ред.пресеты");
            break;
        default:

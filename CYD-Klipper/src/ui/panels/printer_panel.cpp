...
static void update_printer_status_text(lv_event_t * e) 
{
    ...
    if (config == get_current_printer_config())
    {
        switch(global_config.language){
            case 1:
                lv_label_set_text(label, "Под управлением");
                break;
            default:
    ...
    if (printer->state == PRINTER_STATE_OFFLINE)
    {
        switch(global_config.language){
            case 1:
                lv_label_set_text(label, "Оффлайн");
                break;
            default:
...
static void btn_printer_rename(lv_event_t * e)
{
    ...
    switch(global_config.language){
            case 1:
                lv_create_keyboard_text_entry(keyboard_callback, "Переим.Принтер", LV_KEYBOARD_MODE_TEXT_LOWER, CYD_SCREEN_WIDTH_PX * 0.75, 24, keyboard_config->printer_name, false);
                break;
            default:
...
void create_printer_ui(PRINTER_CONFIG * config, lv_obj_t * root)
{
    ...
    switch(global_config.language){
            case 1:
                lv_label_set_text(label, "Переим.");
                break;
            default:
...
void printer_panel_init(lv_obj_t* panel) 
{
    ...
    if (get_printer_config_free_index() != -1){
        ...
        switch(global_config.language){
            case 1:
                lv_label_set_text(label, "Добав.принтер");
                break;
            default:

...
void switch_printer_init() {
    ...
    for (int i = 0; i < PRINTER_CONFIG_COUNT; i++){
        ...
        if (config == get_current_printer_config() && config->ip_configured)
        {
            switch(global_config.language){
                case 1:
                    lv_create_custom_menu_label(printer_name, parent, "Активен");
                    break;
                default:
        ...
        if (config->ip_configured) {
            ...
            switch(global_config.language){
                case 1:
                    lv_create_custom_menu_button(printer_name, parent, btn_switch_printer, (httpCode == 200) ? "Переключ." : "Оффлайн", config);
                    break;
                default:
